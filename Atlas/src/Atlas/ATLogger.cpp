...
namespace Atlas
{
   ...
   void CATLogger::Shutdown()
   {
      ...
      if (CHECK_BIT(m_ucFlags,eFlags::LOGFILE) && !m_vpFileQueue->empty())
      {
         std::ofstream fout(m_sOutputFilename.getCstr());
         std::vector<CString>::iterator iter = m_vpFileQueue->begin();
         std::vector<CString>::iterator iterEnd = m_vpFileQueue->end();
         while (iter != iterEnd)
         {
      ...
      delete m_vpFileQueue;  //Clean up your memory!!!
   }
   ...
   void CATLogger::info(const CString sMessage, ...)
   {
      ...
      if (CHECK_BIT(m_ucFlags,eFlags::LOGFILE))  //If we're outputting to file then store in vector for later processing.
         m_vpFileQueue->push_back(sResult);
